...
export default async function AdvicePage() {
  ...
  const advice = await generateAiAdvice(summaryNutrition, "오늘 식단", daily);
      ...
      <main className="space-y-5 px-4 py-5">
        {!user ? (
          <div className="rounded-2xl bg-white p-5 text-center shadow-sm">
            ...
            <Link href="/login" className="mt-3 inline-block font-semibold text-accent">
              로그인하기
            </Link>
          </div>
        ) : logCount === 0 ? (
          <div className="rounded-2xl bg-white p-5 text-center shadow-sm">
            ...
            <Link href="/" className="mt-3 inline-block font-semibold text-accent">
              메뉴 검색하기
            </Link>
          </div>
        ) : (
          <div className="rounded-2xl bg-white p-5 shadow-sm">
            <h2 className="mb-3 font-bold">오늘 누적 ({logCount}개)</h2>
